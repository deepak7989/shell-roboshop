...
get_instance_id(){
    name=$1
    aws ec2 describe-instances --filters "Name=tag:Name,Values=roboshop" "Name=instance-state-name,Values=running" --query  "Reservations[0].Instances[0].InstanceId" --output text
}
...
do
    ...
    if [ $ACTION == "create" ]; then
        if [ $INSTANCE_ID == "None" ]; then
            ...
            echo "Launched Instnace: $INSTANCE_ID"
        else
